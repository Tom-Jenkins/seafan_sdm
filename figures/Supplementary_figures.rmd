---
title: "Predicting habitat suitability and range shifts under projected climate change for two octocorals in the north-east Atlantic"
subtitle: "Supplementary Figures"
author: "Jenkins and Stevens (in review)"
output:
  pdf_document:
    highlight: tango
    number_sections: no
    toc: yes
classoption: landscape
---
  
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```
<div id="contents"/>


<!-- # Figure S1 -->
## Figure S1
```{r, include=TRUE, fig.align="left", echo=FALSE, out.width = "100%"}
knitr::include_graphics("./FigureS1A.png")
```
\textbf{Figure S1.} Thematic map of each raster predictor variable considered in the species distribution models.
```{r, include=TRUE, fig.align="left", echo=FALSE, out.width = "100%"}
knitr::include_graphics("./FigureS1Bi.png")
```
\textbf{Figure S1.} Thematic map of each raster predictor variable considered in the species distribution models.
```{r, include=TRUE, fig.align="left", echo=FALSE, out.width = "100%"}
knitr::include_graphics("./FigureS1Bii.png")
```
\textbf{Figure S1.} Thematic map of each raster predictor variable considered in the species distribution models.

<!-- # Figure S2 -->
## Figure S2
```{r, include=TRUE, fig.align="left", echo=FALSE, fig.show="hold", out.width="45%", out.height="80%"}
knitr::include_graphics(c("./gt_psf.png", "./gt_dmf.png"))
```
\textbf{Figure S2.} Results of model evaluation for the pink sea fan (\textit{Eunicella verrucosa}) and dead man's fingers (\textit{Alcyonium digitatum}).
